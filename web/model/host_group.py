...
class HostGroup(Bean):
    _tbl = 'grp'
    _cols = 'id, grp_name, create_user, come_from, maintain_begin, maintain_end'
    _id = 'id'

    def __init__(self, _id, grp_name, create_user, come_from, maintain_begin, maintain_end):
        self.id = _id
    ...
    @classmethod
    def maintain(cls, begin, end, gids):
        if not gids:
            return 'group ids is blank'
        cls.update('maintain_begin = %s, maintain_end = %s where id in (%s)' % (begin, end, gids))
        return ''

    @classmethod
    def no_maintain(cls, gids):
        if not gids:
            return 'group ids is blank'
        cls.update('maintain_begin = 0, maintain_end = 0 where id in (%s)' % gids)
        return ''
